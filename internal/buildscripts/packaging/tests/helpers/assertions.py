...
def run_container_cmd(container, cmd, env=None, exit_code=0):
    """
    Runs a container command and tests if the exit code matches exit_code
    """
    print(f"Running '{cmd}' ...")
...
def service_is_running(container, service_name):
    """
    Tests if a systemd service is running
    """
    cmd = f"systemctl status {service_name}"
    return run_container_cmd(container, cmd)


def process_is_running_as_user(container, process, user):
    """
    Tests if a process is running as user
    """
    cmd = f"pgrep -a -u {user} {process}"
    return run_container_cmd(container, cmd)
